...
macro( __DETECT_TOOLCHAIN_MACHINE_NAME _var _root )
 ...
 file( GLOB __clangExePath "${_root}/bin/clang${TOOL_OS_SUFFIX}" )
 __LIST_FILTER( __clangExePath "bin/[.].clang${TOOL_OS_SUFFIX}$" )
 list( LENGTH __clangExePath __clangExePathsCount )
 if( NOT __gccExePathsCount EQUAL 1 )
    if (NOT __clangExePathsCount EQUAL 1)
        if (CMAKE_VERBOSE_MAKEFILE)
            message( WARNING "Could not uniquely determine machine name for compiler from ${_root}." )
        endif()
    else()
        if (CMAKE_VERBOSE_MAKEFILE)
            message(STATUS "Android Clang detected, but will not be used")
        endif()
    endif()
    set( ${_var} "" )
 else()
...
###########################################################################################
# [vblinov]: this is disabled to search libraries and packages outside of toolchain sysroot
###########################################################################################
